...
// Pagination settings
$programsPerPage = 5; // Number of training programs per page
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1; // Current page number
$offset = ($page - 1) * $programsPerPage; // Offset for SQL query

// Fetch total number of training programs
$totalProgramsQuery = "SELECT COUNT(*) AS total FROM training_programs";
$totalProgramsResult = $conn->query($totalProgramsQuery);
$totalPrograms = $totalProgramsResult->fetch_assoc()['total'];

// Fetch training programs for the current page
$query = "SELECT * FROM training_programs LIMIT $offset, $programsPerPage";
$result = $conn->query($query);

// Calculate total pages
$totalPages = ceil($totalPrograms / $programsPerPage);
?>
...
<head>
    ...
    <title>Manage Training Programs</title>
...
<body>
    ...
    <div class="content" id="content">
        <h1 class="text-center ">Manage Training Programs</h1>
        <div class="d-flex justify-content-start mb-3">
            <a href="../add/add_training.php" class="btn btn-success"><i class="fa fa-plus me-2"></i>Add New Training Program</a>
        </div>
...
<td>
                                ...
                                <a href='../../pages/veiw/training-viewer.php?id={$row['id']}' class='text-primary'>Read More</a>
                            </td>                            <td>{$row['duration']}</td>
                            <td>
                                <a href='../../{$row['file_url']}' target='_blank' class='btn btn-info btn-sm'>
                                    <i class='fa fa-download'></i> Download
                                </a>
                            </td>
                            <td>
                                <a href='../edit/edit_training.php?id={$row['id']}' class='btn btn-primary btn-sm'>
                                    <i class='fa fa-edit'></i> Edit
                                </a>
                                <a href='../delete/delete_training.php?id={$row['id']}' class='btn btn-danger btn-sm' onclick='return confirm(\"Are you sure you want to delete this training program?\")'>
                    ...
                    echo "<tr><td colspan='6' class='text-center text-danger'>No training programs found.</td></tr>";
                }
                ?>
            </tbody>
        </table>
