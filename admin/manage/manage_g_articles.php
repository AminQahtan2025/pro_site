...
$query = "SELECT * FROM G_articles LIMIT $offset, $articlesPerPage";
...
<head>
    ...
    <title>Manage General Articles</title>
...
<body>
    ...
    <div class="content" id="content">
        <h1 class="text-center ">Manage General Articles</h1>
        <div class="d-flex justify-content-start mb-3">
            <a href="../add/add_g_article.php" class="btn btn-success"><i class="fa fa-plus me-2"></i>Add New Article</a>
        </div>
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        // Fetch the number of comments and ratings for this article
                        $articleId = $row['id'];
                        $commentsQuery = "SELECT COUNT(*) AS total_comments, AVG(rating) AS avg_rating FROM g_article_comments WHERE article_id = $articleId";
                        $commentsResult = $conn->query($commentsQuery);
                        $commentsData = $commentsResult->fetch_assoc();
                        $totalComments = $commentsData['total_comments'];
                        $avgRating = $commentsData['avg_rating'] ? round($commentsData['avg_rating'], 1) : 'No ratings';
                        ...
                        <tr>
                            ...
                            <td>
                                <a href='manage_comments.php?article_id={$row['id']}' class='btn btn-info btn-sm'>
                                    Comments ($totalComments) | Rating: $avgRating
                                </a>
                            </td>
                            <td>
                                <a href='../edit/edit_g_article.php?id={$row['id']}' class='btn btn-primary btn-sm'>
                                    <i class='fa fa-edit'></i> Edit
                                </a>
                                <a href='../delete/delete_g_articale.php?id={$row['id']}' class='btn btn-danger btn-sm' onclick='return confirm(\"Are you sure you want to delete this article?\")'>
                    ...
                    echo "<tr><td colspan='6' class='text-center text-danger'>No articles found.</td></tr>";
                }
                ?>
            </tbody>
        </table>
