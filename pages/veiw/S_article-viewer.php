...
if ($article_id) {
    ...
    $stmt = $conn->prepare("SELECT * FROM s_articles WHERE id = ?");
    ...
    if ($article) {
        ...
        $updateViewsQuery = $conn->prepare("UPDATE s_articles SET views = views + 1 WHERE id = ?");
        $updateViewsQuery->bind_param("i", $article_id);
        $updateViewsQuery->execute();
        // Fetch other articles for the sidebar or suggestions
        $stmt_other = $conn->prepare("SELECT id, title FROM s_articles WHERE id != ? LIMIT 5");
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($user_name && $rating && $comment) {
                ...
                $stmt_comment = $conn->prepare("INSERT INTO s_article_comments (article_id, user_name, rating, comment) VALUES (?, ?, ?, ?)");
                ...
                if ($stmt_comment->execute()) {
                    ...
                    $stmt_update_comments = $conn->prepare("UPDATE s_articles SET comments = comments + 1 WHERE id = ?");
...
<head>
    ...
    <title>Scintific Article Viewer</title>
...
<body>
   <div class="container">
       <div class="banner border mt-3">
            <h1 class="text-center m-3">Scintific Article Viewer</h1>
       </div>
       <!-- Breadcrumb Navigation -->
       <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="../../index.html">Home</a> / </li>
                <li class="breadcrumb-item"><a href="../G_articles.php">Scintific Articles</a> / </li>
                <li class="breadcrumb-item" aria-current="page">Scintific Article Viewer</li>
            </ol>
       </nav>
       <!-- Article Section -->
       <div class="row">
           ...
           <div class="col-md-3">
               <div class="article border">
                   <div class="section-header text-white p-2 rounded-top">
                       <h5>Other Scintific Articles</h5>
                   </div>
                   <?php if (isset($error_message)): ?>
                       <div class="alert alert-danger"><?= $error_message ?></div>
                   <?php else: ?>
               ...
               <div class="article border rounded shadow-sm mt-2">
                   ...
                   <div class="article-body p-2">
                       ...
                       $stmt_comments = $conn->prepare("SELECT * FROM s_article_comments WHERE article_id = ? ORDER BY created_at DESC");
