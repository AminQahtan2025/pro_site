...
if ($program_id) {
    ...
    if ($program) {
        // Increment the views count for the program
        $updateViewsQuery = $conn->prepare("UPDATE training_programs SET views = views + 1 WHERE id = ?");
        $updateViewsQuery->bind_param("i", $program_id);
        $updateViewsQuery->execute();
        // Fetch other programs for the sidebar or suggestions
        $stmt_other = $conn->prepare("SELECT id, program_name FROM training_programs WHERE id != ? LIMIT 5");
        $stmt_other->bind_param("i", $program_id);
        $stmt_other->execute();
        $other_programs = $stmt_other->get_result();
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($user_name && $rating && $comment) {
                ...
                $stmt_comment = $conn->prepare("INSERT INTO program_comments (program_id, user_name, rating, comment) VALUES (?, ?, ?, ?)");
                $stmt_comment->bind_param("isis", $program_id, $user_name, $rating, $comment);

                if ($stmt_comment->execute()) {
                    // Increment the comment count for the program
                    $stmt_update_comments = $conn->prepare("UPDATE training_programs SET comments = comments + 1 WHERE id = ?");
        ...
        $filePath = "../../" . htmlspecialchars($program['file_url']);
        ...
        // program not found
        $error_message = "<div class='alert alert-danger'>Program not found. Please check the ID.</div>";
    }
} else {
    // Invalid or missing program ID
    $error_message = "<div class='alert alert-danger'>Invalid Program ID. Please provide a valid ID.</div>";
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Trianing Program Viewer</title>
...
<body>
   <div class="container">
       <div class="banner border mt-3">
            <h1 class="text-center m-3">Trianing Program Viewer</h1>
       </div>
       <!-- Breadcrumb Navigation -->
       <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="../../index.html">Home</a> / </li>
                <li class="breadcrumb-item"><a href="../training.php">Trianing Programs</a> / </li>
                <li class="breadcrumb-item" aria-current="page">Trianing Program Viewer</li>
            </ol>
       </nav>
       <!-- program Section -->
       <div class="row">
        ...
        <div class="article border">
            ...
            <?php else: ?>
                ...
                <div class="article-header p-3">
                    <h1 class="article-title mb-3"><?= htmlspecialchars($program['program_name']) ?></h1>
                    <div class="article-meta d-flex align-items-center mb-3">
                        ...
                        <div class="d-flex flex-column">
                            <span class="article-author fw-bold"><?= htmlspecialchars($program['author'] ?? "Dr. Abdullah Al-Swidi") ?></span>
                            <span class="text-muted"><?= htmlspecialchars($program['created_at']) ?></span>
                        </div>
                    </div>
                    <hr class="my-2">
                    <!-- program Stats -->
                    <div class="article-stats d-flex justify-content-between align-items-center pe-2">
                        <div class="d-flex gap-3">
                            <span class="views text-muted">
                                <i class="fa fa-eye me-1"></i> <?= htmlspecialchars($program['views'] ?? "0") ?> Views
                            </span>
           ...
           <div class="col-md-3">
               <div class="article border">
                   ...
                   <div class="article-body">
                       <ul class="list-group">
                           <?php while ($other = $other_programs->fetch_assoc()): ?>
                               <li class="list-group-item">
                                   <a href="?id=<?= $other['id'] ?>" class="text-decoration-none text-primary">
                                       <i class="fa fa-file-text"></i> <?= htmlspecialchars($other['program_name']) ?>
                                   </a>
                               </li>
                           <?php endwhile; ?>
                       </ul>
               ...
               <div class="program border rounded shadow-sm mt-2">
                   ...
                   <div class="program-body p-2">
                       ...
                       $stmt_comments = $conn->prepare("SELECT * FROM program_comments WHERE program_id = ? ORDER BY created_at DESC");
                       $stmt_comments->bind_param("i", $program_id);
