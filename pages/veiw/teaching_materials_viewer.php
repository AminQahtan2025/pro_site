...
if ($material_id) {
    // Fetch the main program
    $stmt = $conn->prepare("SELECT * FROM teaching_materials WHERE id = ?");
    ...
    if ($material) {
        // Increment the views count for the material
        $updateViewsQuery = $conn->prepare("UPDATE teaching_materials SET views = views + 1 WHERE id = ?");
        $updateViewsQuery->bind_param("i", $material_id);
        $updateViewsQuery->execute();
        // Fetch other materials for the sidebar or suggestions
        $stmt_other = $conn->prepare("SELECT id, title FROM teaching_materials WHERE id != ? LIMIT 5");
        $stmt_other->bind_param("i", $material_id);
        $stmt_other->execute();
        $other_materials = $stmt_other->get_result();
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($user_name && $rating && $comment) {
                ...
                $stmt_comment = $conn->prepare("INSERT INTO material_comments (material_id, user_name, rating, comment) VALUES (?, ?, ?, ?)");
                $stmt_comment->bind_param("isis", $material_id, $user_name, $rating, $comment);

                if ($stmt_comment->execute()) {
                    // Increment the comment count for the material
                    $stmt_update_comments = $conn->prepare("UPDATE teaching_materials SET comments = comments + 1 WHERE id = ?");
                    $stmt_update_comments->bind_param("i", $program_id);
        ...
        // (Assuming the database field is 'file_url' even though it may be any file type)
        $filePath = "../../" . htmlspecialchars($material['file_url']);
        ...
        // program not found
        $error_message = "<div class='alert alert-danger'>Teaching Material not found. Please check the ID.</div>";
    }
} else {
    // Invalid or missing program ID
    $error_message = "<div class='alert alert-danger'>Invalid Teaching Material ID. Please provide a valid ID.</div>";
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Teaching Materials Viewer</title>
...
<body>
   <div class="container">
       <div class="banner border mt-3">
            <h1 class="text-center m-3">Teaching Materials Viewer</h1>
       </div>
       <!-- Breadcrumb Navigation -->
       <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="../../index.html">Home</a> / </li>
                <li class="breadcrumb-item"><a href="../materials.php">Teaching Materials</a> / </li>
                <li class="breadcrumb-item" aria-current="page">Teaching Materials Viewer</li>
            </ol>
       </nav>
       <!-- program Section -->
       <div class="row">
    <div class="col-md-9">
        <div class="article border">
            ...
            <?php else: ?>
                ...
                <div class="article-header p-3">
                    <h1 class="article-title mb-3"><?= htmlspecialchars($material['title']) ?></h1>
                    <h6 class="article-category fw-bold"> Category: <?= htmlspecialchars($material['category']) ?></h6>
                    <div class="article-meta d-flex align-items-center mb-3">
                        ...
                        <div class="d-flex flex-column">
                            ...
                            <span class="text-muted"><?= htmlspecialchars($material['created_at']) ?></span>
                        </div>
                    </div>
                    <hr class="my-2">
                    <!-- program Stats -->
                    <div class="article-stats d-flex justify-content-between align-items-center pe-2">
                        <div class="d-flex gap-3">
                            <span class="views text-muted">
                                <i class="fa fa-eye me-1"></i> <?= htmlspecialchars($material['views'] ?? "0") ?> Views
                            </span>
                            <span class="comments text-muted">
                                <i class="fa fa-comment me-1"></i> <?= htmlspecialchars($program['comments'] ?? "0") ?> Comments
                            </span>
                        </div>
                ...
                <div class="article-content">
                    <?php if (isset($htmlContent)): ?>
                        <?= $htmlContent ?>
                    <?php else: ?>
                        <p class="text-muted">No content available for this program.</p>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
        </div>
        <hr class="my-4">
               ...
               <h3 class="section-header rounded-top">Leave a Comment and Rate the program:</h3>
           ...
           <div class="col-md-3">
               <div class="article border">
                   ...
                   <div class="article-body">
                       <ul class="list-group">
                           <?php while ($other = $other_materials->fetch_assoc()): ?>
               ...
               <!-- Comments Section -->
               <div class="program border rounded shadow-sm mt-2">
                   <div class="section-header text-white p-2 rounded-top">
                       <h5>Comments and Ratings</h5>
                   </div>
                   <div class="program-body p-2">
                       <?php
                       // Fetch comments and ratings for the current program
                       $stmt_comments = $conn->prepare("SELECT * FROM material_comments WHERE material_id = ? ORDER BY created_at DESC");
                       $stmt_comments->bind_param("i", $material_id);
   ...
   <script type="text/javascript">
       ...
       function shareOnFacebook() {
           const facebookUrl = "https://www.facebook.com/sharer/sharer.php?u=" + encodeURIComponent(programUrl);
           window.open(facebookUrl, '_blank', 'width=600,height=400');
       }
       function shareOnTwitter() {
           const twitterUrl = "https://twitter.com/intent/tweet?url=" + encodeURIComponent(programUrl) + "&text=Check out this program!";
