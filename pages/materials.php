...
<body>
    <div class="container">
        <!-- Banner Image -->
        <div class="banner">
            <img src="../images/banner.jpeg" alt="Training programs Banner" class="img-fluid w-100 shadow-sm">
        </div>
        ...
        <div class="container border">
            <h1 class="text-center m-4">Teaching Materials</h1>
            <div class="row g-3">
                <?php
                ...
                $totalArticlesQuery = "SELECT COUNT(*) AS total FROM teaching_materials";
                $totalArticlesResult = $conn->query($totalArticlesQuery);
                $totalArticles = $totalArticlesResult->fetch_assoc()['total'];

                // Fetch articles for the current page
                $sql = "SELECT * FROM teaching_materials LIMIT $offset, $articlesPerPage";
                $result = $conn->query($sql);
                // Fetch articles
                $sql = "SELECT * FROM teaching_materials";
                $result = $conn->query($sql);
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo '
                        <div class="col-md-6">
                            <a href="veiw/teaching_materials_viewer.php?id=' . $row['id'] . '" class="text-decoration-none card-link" style="color: black;">
                                <div class="card mb-3 shadow-sm h-wrap">
                                    <div class="row g-0 align-items-center">
                                        <div class="col-2 text-center p-3">
                                                <i class="fa fa-file-text mt-2 fa-3x" style="color: #008080 ;"></i>
                                        </div>
                    ...
                    echo '<hr><div class="alert alert-danger text-center shadow mt-3 w-100"> No Training programs available.</div>';
                }
